...
from ..validation.validation import safe_float, safe_int, clean_apn, extract_best_zip
from ..validation.validation import normalize_column_name, map_column, parse_array, generate_owner_hash, clean_phone
...
def process_unified_row(row: dict, db) -> dict | None:
    try:
        ...
        mapped_row = {}
        for col, value in processed_row.items():
            canonical = map_column(col, REQUIRED_COLUMN_MAPPINGS) or \
                        map_column(col, OPTIONAL_COLUMN_MAPPINGS)
            if canonical:
        ...
        property_zip = extract_best_zip(processed_row, ["property zip 5", "property zip"])
        mailing_zip = extract_best_zip(processed_row, ["mailing zip 5", "mailing zip"])
        if not property_zip or not mailing_zip:
            db.fallback_candidates.insert_one({
                "raw_data": row,
                "reason": "missing_or_invalid_zip",
                "status": "pending",
                "created_at": datetime.now(timezone.utc),
                "source": "unified_csv"
            })
        ...
        for i in range(1, 31):
            ...
            if phone_number:
                phone_id = f"PHONE-{hashlib.sha256(phone_number.encode()).hexdigest()[:8]}"
                phone_docs.append({
                    "phone_id": phone_id,
                    "number": phone_number,
                    "linked_apns": [apn],
        ...
        for field, event_type in known_life_event_fields.items():
            value = mapped_row.get(field, "").strip()
            if not value:
                continue
            event = {
                "apn": apn,
            ...
            if "date" in field or "year" in field or "since" in field:
                try:
                    event["event_date"] = parse(value, dayfirst=False)
        ...
        for tag in tags:
            ...
            for pattern, event_type in TAG_EVENT_PATTERNS.items():
                if re.search(pattern, tag_lower):
                    ...
                    if date_match:
                        try:
                            event["event_date"] = parse(f"01/{date_match.group(1)}", dayfirst=False)
                        except Exception as e:
        ...
        if sale_reasons:
            life_events.append({
                ...
                "source": "Tag Analysis",
                "source_detail": ", ".join(sale_reasons),
        ...
        if last_sold and sale_price:
            try:
                sale_date = parse(last_sold, dayfirst=False)
